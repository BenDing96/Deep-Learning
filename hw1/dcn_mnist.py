...
def main():
    ...
    with tf.name_scope("conv1"):
        ...
        with tf.name_scope("leaky_relu1"):
    ...
    with tf.name_scope("conv2"):
        with tf.name_scope('conv2_weight'):
            W_conv2 = weight_variable([5, 5, 32, 64])
            variable_summaries(W_conv2)
        with tf.name_scope('conv2_bias'):
            b_conv2 = bias_variable([64])
            variable_summaries(b_conv2)
        with tf.name_scope('leaky_relu2'):
    ...
    with tf.name_scope("fully_connected"):
        ...
        with tf.name_scope('fc_flatten'):
            h_pool2_flat = tf.reshape(h_pool2, [-1, 7 * 7 * 64])
            variable_summaries(h_pool2_flat)
        with tf.name_scope('fc_leaky_relu'):
    ...
    with tf.name_scope("softmax"):
        ...
        with tf.name_scope("softmax_conv"):
